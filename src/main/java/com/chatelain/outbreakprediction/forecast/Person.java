...
public class Person {
    ...
    int sig = 1;
    private OutBreakCalculator calculator;
    ...
    public Person(City city, int x, int y, OutBreakCalculator calculator) {
        this.city = city;
        this.x = x;
        this.y = y;
    ...
    public boolean wantMove() {
        double value = sig * new Random().nextGaussian() + calculator.params.getU();
        return value > 0;
    }
    ...
    public void beInfected() {
        state = State.SHADOW;
        infectedTime = calculator.worldTime;
    }
...
//        }
    ...
    public void update() {
        ...
        if (state == State.CONFIRMED && calculator.worldTime - confirmedTime >= calculator.params.getHOSPITAL_RECEIVE_TIME()) {
            Bed bed = calculator.hospital.pickBed();//查找空床位
            if (bed == null) {
                //没有床位了
//                System.out.println("隔离区没有空床位");
        ...
        if (calculator.worldTime - infectedTime > calculator.params.getSHADOW_TIME() && state == State.SHADOW) {
            state = State.CONFIRMED;//潜伏者发病
            confirmedTime = calculator.worldTime;//刷新时间
        }

        action();

        List<Person> people = calculator.personPool.personList;
        ...
        for (Person person : people) {
            ...
            if (random < calculator.params.getBROAD_RATE() && distance(person) < SAFE_DIST) {
                this.beInfected();
            }
        }
    }
